...
type AlicloudDiskDevice struct {
	...
	RawEncrypted       trilean.Trilean `mapstructure:"disk_encrypted"`
...
type AlicloudImageConfig struct {
	...
	RawImageEncrypted                 trilean.Trilean   `mapstructure:"image_encrypted"`
...
func (c *AlicloudImageConfig) Prepare(ctx *interpolate.Context) []error {
	...
	c.ImageEncrypted = trilean.ToBoolPointer(c.RawImageEncrypted)

	c.ECSSystemDiskMapping.Encrypted = trilean.ToBoolPointer(c.ECSSystemDiskMapping.RawEncrypted)
	for i := range c.ECSImagesDiskMappings {
		c.ECSImagesDiskMappings[i].Encrypted = trilean.ToBoolPointer(c.ECSImagesDiskMappings[i].RawEncrypted)
	}
